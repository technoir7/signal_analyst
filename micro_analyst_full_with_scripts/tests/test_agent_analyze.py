...
def client():
    class AuthTestClient(TestClient):
        def post(self, *args, **kwargs):
            ...
            headers.setdefault("X-API-Key", "demo_key_abc123")
            kwargs["headers"] = headers
            return super().post(*args, **kwargs)
    return AuthTestClient(micro_analyst.app)
...
def test_analyze_happy_path(client, company_name, company_url, monkeypatch):
    ...
    def fake_synthesize_report(profile: dict, focus: str | None = None, delta_context=None) -> str:
        return "# Stub report\n\nAll good."
    ...
    # Report is generated by LLM client which was monkeypatched
    assert "Stub report" in body["report_markdown"]
...
def test_analyze_web_scrape_failure_skips_seo_and_tech(
    ...
    # Just verify basic structure - profile is now InferredProfile
...
def test_analyze_planning_failure_uses_default_plan(
    ...
    body = resp.json()
    profile = body["profile"]
    # Profile is now InferredProfile - just verify it has the expected sections
    assert "web" in profile
    assert "tech_stack" in profile
